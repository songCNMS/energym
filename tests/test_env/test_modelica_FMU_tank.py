...
def plot_swiss_house(mdl, record, model_name):
    ...
    record[mdl["inputs"]].plot(ax=ax[row - 1])
...
def run_model(model_name: str):
    ...
    record = pd.DataFrame(
        np.zeros((N, len(mdl["inputs"]) + len(mdl["states"]))),
        index=np.arange(N) * mdl["step_size"],
        columns=mdl["inputs"] + mdl["states"],
    )
    ...
    for dt in record.index:
        ...
        control_step(env, mdl["inputs"], uval)

        # Save into DataFrame
        record.loc[dt, mdl["inputs"]] = [val[0] for val in uval]
